...
public class TurnMarker extends Observable implements Serializable {
	 ...
	 * @author f.bellentani
	 */
	public class TeamMarker extends Observable implements Serializable {
		...
		private boolean usedReroll = false;

		private TurnStatus status = TurnStatus.DONE;

		private TeamMarker(Team team) {
			this.team = team;
		}
	...
	private Map<Team, TeamMarker> markers = new HashMap<Team, TeamMarker>();

	protected TurnMarker(Team... teams) {
		for (Team team : teams) {
			TeamMarker tm = new TeamMarker(team);
			markers.put(team, tm);
		}
	}
