...
public class AgilityTable {
    ...
    public static boolean dodgeRoll(EventManager eventManager, Coach coach, Player player, Direction direction, boolean gfi) {
        ...
        if (gfi) {
            int gfiRoll = Roll.Companion.roll(1, D6, movePlayerEvent, "GFI", player.toString()).setTarget(2).getSum();
            if (gfiRoll < 0) {
                dodged = false;
            }
        }
        ...
        if (dodged) {
            ...
            if (opponentTZCount > 0) {
                RollResult dodge = Roll.Companion.roll(1, D6, movePlayerEvent, "Dodge", player.toString());
    ...
    public static boolean catchRoll(EventManager eventManager, Coach coach, boolean accurate) {
        ...
        RollResult roll = Roll.Companion.roll(1, D6, catchBallEvent, "Catch", player.toString());
    ...
    public static boolean pickupRoll(EventManager eventManager, Coach coach) {
        ...
        RollResult roll = Roll.Companion.roll(1, D6, pickUpBallEvent, "PickUp", player.toString());
