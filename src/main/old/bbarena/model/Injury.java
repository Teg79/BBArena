...
public class Injury implements Serializable {
	...
	public enum InjuryType {
		...
		public static String combine(InjuryType... types) {
			...
			for (InjuryType it : types) {
				if (res.length() > 0) {
					res.append(Injury.Companion.getINJ_TYPE_SEPARATOR());
				}
				res.append(it);
			}
			return res.toString();
	...
	public Injury(String inj) {
		...
		StringTokenizer tok = new StringTokenizer(it, Companion.getINJ_TOKEN_SEPARATOR());

		// Types
		String types = tok.nextToken();
		StringTokenizer typeTok = new StringTokenizer(types, Companion.getINJ_TYPE_SEPARATOR());
		...
		if (tok.hasMoreTokens()) {
			...
			if (InjuryCure.Companion.isHealed(token2)) {
				healed = true;
			} else {
				cause = InjuryCause.valueOf(token2);

				if (tok.hasMoreTokens() && InjuryCure.Companion.isHealed(tok.nextToken())) {
					healed = true;
				}
			}
		}
	...
	public static List<Injury> parseInjury(String inj) {
		...
		StringTokenizer injTok = new StringTokenizer(inj, Companion.getINJ_SEPARATOR());
	...
	public String toString() {
		return getType() + Companion.getINJ_TOKEN_SEPARATOR() + getCause()
				+ (isHealed() ? getCure() : "");
	}
	...
	public String getType() {
		...
		while (it.hasNext()) {
			...
			res.append(t.toString() + (it.hasNext() ? Companion.getINJ_TYPE_SEPARATOR() : ""));
		}
		return res.toString();
	}
