...
public class EventManager implements Serializable {
	...
	public int putEvent(Event event) {
		...
		getLog().debug("Added Event " + event.toString());
		return res;
	}
	...
	public void popEvent() {
		...
		getLog().debug("Removed last Event");
	}
	...
	public int forward() {
		...
		if (_current < _events.size()) {
			...
			getLog().debug("Forward with event " + e.toString()
					+ " starts...");
	...
	public int backward() {
		...
		if (_current > 0 && _events.size() > 0) {
			...
			getLog().debug("Backward with event " + e.getId() + ": " + e.getString()
					+ " starts...");
