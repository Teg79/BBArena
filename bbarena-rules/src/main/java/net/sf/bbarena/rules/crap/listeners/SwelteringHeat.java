...
public class SwelteringHeat extends BaseEventFlowListener {
    ...
    public void beforeDoEvent(Event e) {
        if (e instanceof EndDriveEvent && Weather.getWeather(Weather.WeatherType.SWELTERING_HEAT).equals(e.getArena().getWeather())) {
            Set<Player> playersOnThePitch = e.getArena().getPitch().getPlayers();
